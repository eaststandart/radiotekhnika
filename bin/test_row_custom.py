...
@about Тестовый изолированный модуль предобработки изображений для Obsidian -> Jekyll.
@purpose Доработка логики групповых кастомных изображений (присвоение row-custom классов).
"""

import re

def test_process_row_galleries(markdown_content):
    """
    Каскадный построчный диспетчер обработки всей графики репозитория.
    Логика полностью идентична оригинальной, за исключением генерации классов
    для групповых кастомных базовых картинок.
    """
        ...
        if is_image:
            ...
            for group_line in image_group_lines:
                ...
                if first_key.lower() == 'v':
                    ...
                    # Оптимизированная проверка без избыточных условий сравнения
