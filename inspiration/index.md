---
layout: default
title: Техническое вдохновение
custom_css: ["/css/fair.css"]
---

Коллекция идей, чертежей и находок, которые помогают в техническом творчестве и инженерном поиске.

<div class="fair-list">
    <a href="our-dream.html" class="fair-card">
        <img src="/img/icons/our-dream.webp" class="fair-img-round">
            <h3>Наша мечта ...</h3>
    </a>
        <a href="strontium-atom.html" class="fair-card">
        <img src="/img/icons/strontium-atom.webp" class="fair-img-round">
            <h3>Атом в ионной ловушке</h3>
    </a>
        <a href="through-hole-red-led.html" class="fair-card">
        <img src="/img/icons/through-hole-red-led.webp" class="fair-img-round">
            <h3>Красный светодиод</h3>
    </a>
        <a href="link-to-post.html" class="fair-card">
        <img src="img/your-icon.webp" class="fair-img-round">
            <h3>Краткое название идеи</h3>
    </a>
        <a href="link-to-post.html" class="fair-card">
        <img src="img/your-icon.webp" class="fair-img-round">
            <h3>Краткое название идеи</h3>
    </a>
    <a href="link-to-post.html" class="fair-card">
        <img src="1.webp" class="fair-img-round">
            <h3>Краткое название идеи.</h3>
    </a>
    <a href="link-to-post.html" class="fair-card">
        <img src="1.webp" class="fair-img-round">
            <h3>Краткое название идеи.</h3>
    </a>
    <a href="link-to-post.html" class="fair-card">
        <img src="1.webp" class="fair-img-round">
            <h3>Краткое название идеи.</h3>
    </a>
        <a href="link-to-post.html" class="fair-card">
        <img src="1.webp" class="fair-img-round">
            <h3>Краткое название идеи.</h3>
    </a>
        <a href="link-to-post.html" class="fair-card">
        <img src="1.webp" class="fair-img-round">
            <h3>Краткое название идеи.</h3>
    </a>
            <a href="through-hole-red-led.html" class="fair-card">
        <img src="/img/icons/through-hole-red-led-2.webp" class="fair-img-round">
            <h3>Красный светодиод</h3>
    </a>
        <a href="link-to-post.html" class="fair-card">
        <img src="img/your-icon.webp" class="fair-img-round">
            <h3>Краткое название идеи</h3>
    </a>
            <a href="through-hole-red-led.html" class="fair-card">
        <img src="/img/icons/through-hole-red-led-2.webp" class="fair-img-round">
            <h3>Красный светодиод</h3>
    </a>
        <a href="link-to-post.html" class="fair-card">
        <img src="img/your-icon.webp" class="fair-img-round">
            <h3>Краткое название идеи</h3>
    </a>
</div>


